...
router.post('/subdl/validate', async (req, res) => {
    const provider = providerManager.get('subdl');
    if (!provider) {
        return res.status(503).json({ valid: false, error: 'SubDL provider not available' });
    }
    const { apiKey } = req.body || {};
    if (!apiKey) {
        return res.status(400).json({ valid: false, error: 'API key required' });
